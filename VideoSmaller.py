...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("./src/discord.png"), QtGui.QIcon.Normal,
                       QtGui.QIcon.Off)
    ...
    def OpenFile(self):
        ...
        file_name, _ = QtWidgets.QFileDialog.getOpenFileName(
            MainWindow, "Open Video File", path, "*")
    ...
    def Run_FFmeg(self):
        ...
        self.worker.bitrate = os.path.getsize(
            self.textEdit.toPlainText()) / 4.2
        self.worker.start()
    ...
    def worker_finished(self):
        ...
        new_size = (os.path.getsize(
            f'{os.getcwd()}/{filename.split(".")[0]}_smaller.mp4') * 1000)
        message = QtWidgets.QMessageBox()
        message.setStyleSheet("color: rgb(255, 255, 255);\n"
                              "background-color: rgb(7, 7, 22);\n")
        message.setWindowTitle("Done!")
        message.setText(
            f"{str(old_size)[:1]+'.'+str(old_size)[1:3]} --> {str(new_size)[:1]+'.'+str(new_size)[1:3]}"
        )
        message.exec_()

    def OpenDiscord(self):
        QtGui.QDesktopServices.openUrl(
            QtCore.QUrl("https://discord.gg/3pP7pDAccn"))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkBox.setToolTip(
            _translate(
                "MainWindow",
                '<html><head/><body><p><span style=" font-size:10pt; color:#ffffff;">remove audio</span></p></body></html>',
            ))
        self.checkBox.setText(_translate("MainWindow", "Remove Audio"))
        self.label.setText(
            _translate(
                "MainWindow",
                '<html><head/><body><p align="center"><span style=" font-size:22pt;">Video smaller</span></p></body></html>',
            ))
...
class FFMEG_Worker(QtCore.QThread):
    ...
    def run(self):
        if self.noAudio:
            os.system(
                f'ffmpeg -i "{self.filename}" -vcodec libx265 -crf 28"{self.filename.split(r"/")[-1].split(".")[0]}_smaller.mp4"'
            )
            return
        os.system(
            f'ffmpeg -i "{self.filename}" -vcodec libx265 -crf 28 "{self.filename.split(r"/")[-1].split(".")[0]}_smaller.mp4"'
